...
* *OAuth authentication*: In addition to a ServiceNow URL, a valid username, and a password, provide a *ClientId* and *Client Secret*to generate a bearer access token with an expiration date.
...
* xref:README.adoc#authentication-methods[Valid credentials] and the ServiceNow host URL are required.
* The *cr-number* auto-generated during change request creation.
* Provide at least one of the following options to poll for these fields until they reach the specified values:
...
| String
| Yes
| The unique number is auto-generated during change request creation.
...
| The *approval* field's value in ServiceNow indicates the change request is approved.
...
| `approval`
| String
| The value of the Approval field in the change request.
...
The following is an example referring to the output parameter 'number' from link:https://github.com/cloudbees-io/servicenow/blob/main/README.adoc#usage-examples-creating[create] step as input in poll-for-approval step:
